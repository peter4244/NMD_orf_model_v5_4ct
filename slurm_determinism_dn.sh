...
#SBATCH --job-name=determinism_dn
#SBATCH --output=results_deposit_h5_2026-08-04/determinism_dn_%j.log

# DEPOSIT-NATIVE port of drivers/slurm_determinism.sh, 2026-08-12.
...
# --config is REQUIRED; without it argparse exits 2 before anything is tested.
$PY verify_determinism.py --config config_dn.yaml --steps 5 --atg "$ATG" --stop "$STOP"
rc=$?; echo "=== determinism exit: $rc ==="; exit $rc
