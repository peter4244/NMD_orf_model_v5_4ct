...
the start codon and the stop codon, together with per-ORF structural features. It is trained on
NMD classifications derived from four primary lung cell types: alveolar type 2 (AT2), large airway
...
interpretation export live in the Zenodo source-data record
([10.5281/zenodo.21544336](https://doi.org/10.5281/zenodo.21544336)). Nothing under `results_4ct/`
is version-controlled.

> **To run this model as part of reproducing the paper, follow `REPRODUCTION.md` §5.7 in the
> analysis repository** — https://github.com/peter4244/nmd_lung_longread_reproduction. It owns the
> run order, the environment, the results-directory choices, and the flags whose omission fails
> silently. This page describes the model itself and does not repeat that.
>
> **You do not need to run any of it to reproduce the paper's §5 numbers.** The deposit's
> `model.zip` carries the trained weights, the per-isoform predictions and the full
...
properties of the deposited artifacts and should be read from them:
`metrics_atg1000_stop1000_seed42_test_clean.json` for the test metrics, and
`predictions_atg1000_stop1000_seed42_all.tsv` for the split sizes and label counts.
...
Up to K=5 candidate ORFs per transcript pass through an `ORFEncoder` with weights shared across
the K ORFs, aggregated by learned attention. Within each encoder, three sub-encoders run in
parallel and share no weights: a CNN over the start window, a CNN over the stop window, and a
structural branch over the per-ORF features.

Full methods are the paper's Supplemental Methods, section "Deep Learning Model"; the layer
definitions are `model.py`. Read `RETRAIN_ARCHITECTURE_CHANGES.md` before changing anything about
the architecture or retraining.

**One consequence matters for reading the interpretation outputs.** The first convolution spans
all nine input channels at once, so no channel has an independent pathway through a branch. The
branch decomposition therefore attributes importance to **regions** — start window, stop window,
...
Everything the pipeline needs is at the top level; work that is not part of it sits in a folder.

| Where | What |
|---|---|
| top level | the pipeline — the numbered scripts, `model.py`/`utils.py`, both configs, `export_rds.R`, the report, the verification harnesses, and the `slurm_*_dn.sh` deposit-native drivers |
...
Every stage has a **deposit-native** driver — `slurm_*_dn.sh`, reading `config_dn.yaml`, writing
to `results_deposit_h5_2026-08-04`, and taking the window from `paths_config.py --selected-tag`
rather than a literal. Submit from the repository root. `slurm_deepshap_all_dn.sh` does joint,
structural and `atg stop` in one job. `slurm_fshap_dn.sh` and `slurm_ensemble_eval_dn.sh` cover
`12_feature_shap_structural.py` and the ensemble evaluation, which are not part of the published
figures.

**`scripts/patch_stop_codon.py` is not part of the pipeline.** It reads `h5["w500/stop_windows"]`,
and a current HDF5 carries `w100`/`w1000`/`w2000` with datasets named `atg_codes`/`stop_codes`, so
it exits 1 on a schema this pipeline no longer produces. It fed SF38's stop-codon usage figure and
...
`paths_config.py --selected-tag` rather than naming them. A driver that names a literal can run to
completion, exit 0, and produce numbers from the wrong window or the wrong results tree — which is
...
input TSVs, and every interpretation export are regenerated by running the pipeline.
...
`paralog_genes.tsv`, `orf_scan_metadata.json`) are produced by **`export_rds.R`**, which reads the
Isopair analysis-cache objects. It also writes `tx_summary_provenance.json` beside
`tx_summary.tsv`, which `data_prep.py` requires before it will build an HDF5.

> **`export_rds.R` is the sole writer of `tx_summary.tsv`**, so there is one writer per artifact
> and the labels are unambiguously the scan's. What that does *not* settle is **which** scan — the
> hazard `tx_summary_provenance.json` exists to close. It records the scan's identity and class
> counts, so a stale isoform universe is detectable rather than assumed.

The HDF5 dataset (`nmd_orf_data.h5`) and `selected_orfs.tsv` are built by **`data_prep.py`**.

To render `orf_model_report_v5.Rmd` you must run the full pipeline first — the report reads
regenerated `results_4ct/` exports that are not shipped.

## Data provenance

- **Sequences:** SQANTI-corrected FASTA from the Isopair pipeline
- **ORF and structural features:** `export_rds.R`, from the Isopair analysis cache
...
- **Trained model and interpretation exports:** Zenodo,
  [10.5281/zenodo.21544336](https://doi.org/10.5281/zenodo.21544336)
- **This repository is archived at** [10.5281/zenodo.21536501](https://doi.org/10.5281/zenodo.21536501)
