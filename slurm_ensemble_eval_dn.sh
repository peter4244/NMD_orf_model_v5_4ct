...
#SBATCH --job-name=ensemble_eval_dn
#SBATCH --output=results_deposit_h5_2026-08-04/ensemble_eval_dn_%j.log

# DEPOSIT-NATIVE port of drivers/slurm_ensemble_eval.sh, 2026-08-12.
...
$PY ensemble_evaluate.py --config config_dn.yaml --results-dir "$RESULTS_DIR" \
    --atg-window "$ATG" --stop-window "$STOP"
rc=$?; echo "=== ensemble exit: $rc ==="; exit $rc
