"""
probe_elevated_composition_profile.py — base composition by distance from an
elevated position, out to +/-60.

WHY THE WIDTH MATTERS. Channel 5 is a rolling GC fraction over +/-25, so an
encoding artifact would hold its enrichment out to 25 and show a shoulder at the
window edge. A local sequence context decays within a few bases. The profile
decides between them without needing a background set: the reference is the same
transcripts' own overall composition.

This is also the measurement that corrected the AU reading. Composition at the
elevated position alone is GC-flat and keto-skewed; the AU enrichment lives in the
FLANKS, and reporting the centre while describing the whole signal from it was the
error.
"""
