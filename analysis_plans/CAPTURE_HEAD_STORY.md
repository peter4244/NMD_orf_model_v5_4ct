...
| ⚠ | **C18–C21 levels are unweighted means over a stratified bank.** `sampling_weight` exists and no recovery producer applies it, so the levels describe the bank's composition rather than the pool's. **Magnitudes only.** C21's direction is definitional — GENCODE assigns `nonsense_mediated_decay` on the annotated CDS terminating prematurely, so uORF-driven decay is annotated `protein_coding` regardless of what we sample. C18's contrast is the least exposed; C19's +0.091 the most. | instrument | — | `build_ism_subset.py`, `build_ism_bank.py:818-832` |
