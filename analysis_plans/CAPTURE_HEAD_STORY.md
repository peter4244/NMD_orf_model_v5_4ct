...
| C22 | **The stratified bank's reweighting exposure is bounded, and the rows this work quotes are safe.** Weights move a within-group mean only insofar as they vary within the group. `nonsense_mediated_decay` — where every headline number is scored — is **98.4% at weight ≈1** with 18 heavy transcripts, worst-case shift **+0.095** adversarially and far less in practice, because GENCODE's NMD biotype is near-coextensive with the `main_orf_stop` strata and those took weight ~1. **`protein_coding` is NOT safe**: 66.4% at weight > 10, worst case **+0.295**, so the posterior's −0.253 on that row needs weights before it is quoted. C18–C21 stand; no recovery producer applies `sampling_weight`, which remains true and now has a bound on what it costs. | instrument | bank n = 4,999; NMD-biotype 1,125, protein_coding 1,332 | `model_reweight_exposure.py`, local, `runlog_model_reweight_exposure_local.txt` |
| C18 | **The gate signature holds on the right target, and is stronger there.** Scored against GENCODE's own `nonsense_mediated_decay` call rather than the annotated main ORF: the head's own argmax recovers **0.304**, the queue built from that same head **0.793** — **+0.489**, gene-clustered CI **[+0.455, +0.522]**, winning 546 transcripts and losing 9. Supersedes the main-ORF demonstration (0.305 / 0.697) that §1 rested on until 2026-08-02. The mechanism itself is read off `model_v6.py` and is target-independent; this is its *demonstration*. | model | n = 1,099 NMD-biotype transcripts in 915 genes; anchor `cand_is_gencode_start == 1` | `model_gate_vs_ranker_gencode.py` job 8900942; `model_gate_margin_paired.py` job 8900950 |
| C19 | **The queue beats pure position, but only modestly — and far less than the wrong target suggested.** `queue − most-5′` is **+0.091**, gene-clustered CI **[+0.053, +0.129]**, 241 won against 141 lost. On the main-ORF target the same contrast looked like +0.273. **Most of the queue's accuracy is reachable with no model at all**, since position alone scores 0.702. Intervals are gene-clustered bootstraps over 915 genes; McNemar (p = 3.5e−07) is the optimistic bound and is in the runlog. | model | as C18 | `model_gate_margin_paired.py` job 8900950 |
| C20 | **The floor under the 0.883 headline is 0.460.** Longest-ORF recovery on the same NMD-biotype transcripts. The prior's margin over it is **+0.333**, CI **[+0.298, +0.367]**. Closes a gap marked `[unclaimed]` through 2026-08-02; the previously quoted 0.678 belongs to main-ORF recovery and does not apply to this target. | model | as C18 | `model_gate_vs_ranker_gencode.py` job 8900942 |
| C21 | **The GENCODE NMD benchmark contains one NMD mechanism, not both, so 0.883 is scoped.** On NMD-biotype transcripts position recovers **0.702** and length only **0.460**; on `protein_coding` the pattern inverts to 0.493 and 0.958. The decay-causing frame there therefore starts at the normal start codon with nothing in front of it and is *truncated* by the premature stop codon — premature stop in the main frame. A transcript whose decay is uORF-driven keeps an intact main CDS and is annotated `protein_coding`, so **ATF4 is in the contrast row, not the benchmark row**. **0.883 is accuracy on premature stop codon in the main frame.** | model | as C18, plus n = 1,285 `protein_coding` in 1,165 genes | `model_gate_vs_ranker_gencode.py` job 8900942 |
| C17 | **The capture head became decay-predictive within its information limit.** `capture ~ p_decay` is **+0.362** among short candidates (<200 nt) and **+0.400** holding the `n_downstream_ejc` column fixed, so it is not a re-derivation of the capture–junction correlation. This is the *measured* leg of the architecture-verified / mechanism-derived / consequence-measured account of how the objective couples the two heads. | model | within transcript, n = 4,293 short-candidate transcripts; tie-proof n = 3,982 | jobs 8900114, 8900473 |
