...
| ⚠ | **C18–C21 ARE UNWEIGHTED SUBSET STATISTICS.** The bank is stratified with `sampling_weight` recorded per transcript and its own h5 attribute states that population-level estimates must be reweighted. No recovery producer reads that column. **C21 is additionally at risk in kind, not just in degree**: the strata are `{NMD, control} × {main-ORF stop, NO main-ORF stop, no annotation}`, so the bank's composition along the premature-stop-in-main-ORF axis — the exact thing C21 interprets — was set by the sampling design. Suspended pending recompute. | instrument | — | `build_ism_subset.py`, `build_ism_bank.py:818-832` |
